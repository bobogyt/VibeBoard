...
/** 校验并规整项目字段;非法返回错误文案 */
function validateProject(body: Record<string, unknown>, { partial = false } = {}): string | null {
...
function normalizeTechStack(tech_stack: unknown): string | null {
...
function rowToProject(row: ProjectRow): Project {
  ...
  return {
    ...
    techStack: row.tech_stack ? (JSON.parse(row.tech_stack) as string[]) : [],
    ...
    totalCount,
    doneCount,
    progress: totalCount > 0 ? Math.round((doneCount / totalCount) * 100) : null,
  }
}
...
export async function getProject(userId: string, projectId: string): Promise<Project> {
  ...
  const list = rows as unknown as ProjectRow[]
  if (list.length === 0) throw httpError(404, '项目不存在')
  return rowToProject(list[0])
}

export async function createProject(userId: string, body: Record<string, unknown>): Promise<Project> {
  ...
  await pool.execute(
    ...
    [
      ...
      (body.status as string) ?? 'planning',
      (body.repo_url as string | null) ?? null,
      normalizeTechStack(body.tech_stack),
      (body.start_date as number | null) ?? null,
      (body.due_date as number | null) ?? null,
      now,
      now,
    ],
  )
...
export async function updateProject(userId: string, projectId: string, body: Record<string, unknown>): Promise<Project> {
  ...
  const next = {
    ...
    description: body.description !== undefined ? (body.description as string) : (existing.description ?? ''),
    status: body.status !== undefined ? (body.status as string) : existing.status,
    repo_url: body.repo_url !== undefined ? (body.repo_url as string | null) : existing.repo_url,
    tech_stack: body.tech_stack !== undefined ? normalizeTechStack(body.tech_stack) : existing.tech_stack,
    start_date: body.start_date !== undefined ? (body.start_date as number | null) : existing.start_date,
    due_date: body.due_date !== undefined ? (body.due_date as number | null) : existing.due_date,
  }
  await pool.execute(
...
export async function deleteProject(userId: string, projectId: string): Promise<{ ok: boolean }> {
  const [rows] = await pool.execute('SELECT id FROM projects WHERE id = ? AND owner_id = ?', [projectId, userId])
  if ((rows as unknown[]).length === 0) throw httpError(404, '项目不存在')
