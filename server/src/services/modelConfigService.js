 ...
 */
export async function resolveModelConfig(userId) {
...
export async function getAgentModelsInfo(userId) {
  const [rows] = await pool.execute(
...
export async function saveModelConfig(userId, { provider, model, apiKey } = {}) {
  ...
  const keyToStore = trimmedKey ?? rows[0]?.api_key ?? null
  ...
  await pool.execute(
    ...
    [userId, provider, keyToStore, model.trim(), now, now],
  )
...
export async function deleteModelConfig(userId, provider) {
