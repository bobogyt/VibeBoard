 ...
 */
export async function executeTool(tool, rawArguments, ctx = {}) {
  ...
  try {
    return { ok: true, result: await tool.execute(checked.args, ctx) }
  } catch (err) {
    return { ok: false, error: err.message || '工具执行失败' }
  }
}

/** 参数解析与工具级校验;审批前也要走,避免让用户确认一个连参数都不合法的调用 */
export function parseAndValidate(tool, rawArguments) {
  let args
  try {
