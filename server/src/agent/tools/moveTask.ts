...
export const moveTaskTool: Tool = {
  ...
  validate(args) {
    ...
    if (!COLUMN_IDS.includes(args.toStatus as string)) return 'toStatus 需为 todo/doing/done'
    if (args.toPosition !== undefined && args.toPosition !== null && (!Number.isInteger(args.toPosition) || (args.toPosition as number) < 0)) {
      return 'toPosition 需为非负整数'
    }
    return null
  },
  async execute(args, ctx) {
    return { task: await moveTask(ctx.userId as string, args.taskId, args.toStatus, args.toPosition ?? undefined) }
  },
}
