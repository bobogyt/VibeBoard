import { createTasksBatch } from '../../services/taskService.js'
...
export const createTasksTool = {
  ...
  validate(args) {
    if (!Array.isArray(args.tasks)) return 'tasks 需为对象数组'
    if (args.tasks.length === 0) return 'tasks 不能为空'
    if (args.tasks.length > MAX_CREATE_BATCH) return `单次最多创建 ${MAX_CREATE_BATCH} 个任务`
    for (const item of args.tasks) {
      if (typeof item !== 'object' || item === null || Array.isArray(item)) return 'tasks 每项需为对象'
      if (typeof item.title !== 'string' || item.title.trim().length === 0) return '每个任务都需要非空标题'
      if (item.title.length > 200) return '任务标题不能超过 200 字'
    }
    return null
  },
  /** 审批预览:数组顺序即执行顺序,标题带序号;声明了依赖的项标注其前置 */
  preview(args) {
    return args.tasks.map((t, i) => ({
      title: `${i + 1}. ${t.title}${
        t.dependsOn?.length ? `(依赖 ${t.dependsOn.map((d) => `第 ${d + 1} 项`).join('、')})` : ''
      }`,
  ...
  async execute(args, ctx) {
    return createTasksBatch(ctx.userId, args.tasks)
  },
}
