import { batchUpdateTasks } from '../../services/taskService'
...
export const batchUpdateTasksTool: Tool = {
  ...
  validate(args) {
    ...
    const hasPriority = p.priority !== undefined
    const hasProjectId = p.projectId !== undefined
    if (!hasStatus && !hasPriority && !hasProjectId) return 'patch 至少包含 status/priority/projectId 一项'
    if (hasStatus && !COLUMN_IDS.includes(p.status as string)) return 'status 需为 todo/doing/done'
    if (hasPriority && !PRIORITIES.includes(p.priority as string)) return 'priority 需为 P0/P1/P2/P3'
    if (hasProjectId && p.projectId !== null && typeof p.projectId !== 'string') {
      return 'projectId 需为字符串或 null'
    }
    return null
  },
  async execute(args, ctx) {
    return batchUpdateTasks(ctx.userId as string, args.taskIds, args.patch as Record<string, unknown>)
  },
}
