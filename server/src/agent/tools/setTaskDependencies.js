import { getTasks, setTaskDependencies } from '../../services/taskService.js'

export const setTaskDependenciesTool = {
  ...
  async preview(args, ctx) {
    ...
    const all = await getTasks(ctx.userId)
    ...
    const self = byId.get(args.taskId)
    rows.unshift({
  ...
  async execute(args, ctx) {
    return { task: await setTaskDependencies(ctx.userId, args.taskId, args.dependsOn) }
  },
}
