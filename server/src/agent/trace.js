...
export function traceModel(session, { duration, toolCalls, usage }) {
  session.steps.push({
...
export function traceTool(session, { name, args, ok, duration, result, error }) {
  session.steps.push({
    step: session.currentStep,
    type: 'tool',
    name,
...
export function traceFinal(session, totalDuration) {
