...
const STATUS_LABELS: Record<string, string> = { todo: 'Todo', doing: 'Doing', done: 'Done' }

/* eslint-disable @typescript-eslint/no-explicit-any */
function argsOf(args: any): any {
  return args && typeof args === 'object' ? args : {}
}

function taskTitle(result: any): string | null {
  const title = result?.task?.title
  return typeof title === 'string' && title.length > 0 ? title : null
}

/** 生成步骤摘要;取不到目标细节时回退为通用文案,保证 label 永远非空 */
export function stepLabel(name: string, args: any, ok: boolean, result: any): string {
  const a = argsOf(args)
  switch (name) {
    case 'getTasks':
...
function batchPatchParts(args: any): string[] {
  const a = argsOf(args)
  const parts: string[] = []
...
export function intentLabel(name: string, args: any): string {
