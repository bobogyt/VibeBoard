...
> 本仓库是公开仓库,且项目源服务器曾被勒索软件入侵——公网部署前逐项确认:

1. **删除测试账号**:`e2euser` 的账密写在公开仓库内(e2e-check.sh),部署后立即从数据库删除,并清理「演示-」前缀的演示数据
2. **数据库不暴露**:MySQL/Redis 仅绑定 127.0.0.1,防火墙只放行 HTTP(S) 端口;数据库被勒索正是本项目的真实历史
3. **HTTPS 反向代理**:nginx/caddy 终结 TLS;若在代理层取真实 IP 或限速,需在应用设置 `app.set("trust proxy", 1)`,否则限速按代理 IP 计数
4. **配置加密密钥**:`MODEL_KEY_ENCRYPTION_KEY`(`openssl rand -hex 32`),用于模型 API Key 的 AES-256-GCM 静态加密;不配置则从部署凭据派生(轮换数据库密码会使已存 Key 失效)
5. **服务器自身加固**:最小安装、密钥登录、fail2ban、系统与依赖定期更新(`npm audit` 应保持 0 漏洞)
6. **内置防线**(无需配置):登录/注册/Agent 分级限速、全局洪泛限速(240 次/分/IP)、API Key 静态加密、看板规模上限(500 任务/依赖 ≤50)、请求体编码守卫(U+FFFD 拒绝)、安全响应头、自动化只读运行
